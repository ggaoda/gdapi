...
const Index: React.FC = () => {
  ...
  const [imageUrl, setImageUrl] =  useState<string | null>(loginUser?.avatarUrl ?? null);
  ...
  const getSdk = () => {
    window.location.href = errorConfig.baseURL + '/api/interfaceInfo/sdk';
  };
  const showDevKey = () => {
    loadData();
  };
    ...
    <>
      <Space direction="vertical" size="middle" style={{ display: 'flex' }}>
        <Card
          ...
          title="个人信息"
          actions={[
            <b key="gender">性别：{loginUser?.gender === null ? '未知' : (loginUser?.gender === 1 ? '男'  :  '女')}</b>,
            <b key="time">创建时间：{loginUser?.createTime?? null}</b>,
            <b key="role">身份：{loginUser?.role === (0 ?? null) ? '普通用户' : '管理员'}</b>,
          ]}
          extra={
            ...
            avatar={
              <>
                <Upload
                  ...
                  action={errorConfig.baseURL + '/api/user/update/avatar'}
